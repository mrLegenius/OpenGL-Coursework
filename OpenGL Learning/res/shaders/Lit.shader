...
void main()
{
	...
	vec3 specTexture = texture(u_Material.specularMap, TexCoords).rgb;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
	...
	vec3 diffuse = light.diffuse * (u_Material.useDiffuseMap * vec3(texture(u_Material.diffuseMap, TexCoords)) - (u_Material.useDiffuseMap - 1) * u_Material.diffuse);
	vec3 specular = light.specular * (u_Material.useSpecularMap * vec3(texture(u_Material.diffuseMap, TexCoords)) - (u_Material.useSpecularMap - 1) * u_Material.specular);
