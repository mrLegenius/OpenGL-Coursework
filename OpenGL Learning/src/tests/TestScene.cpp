...
namespace test 
{
	test::TestScene::TestScene() : m_LightPos(glm::vec3(0, 1, 0))
	{
		...
		m_WaterShader->SetUniform1i("u_Material.useDiffuseMap", 1);
		m_WaterShader->SetUniform1i("u_Material.useSpecularMap", 0);
		m_WaterShader->SetUniform1i("u_Material.useNormalMap", 0);
		...
		m_WaterShader->SetUniform1f("transparency", 0.5f);
	...
	void test::TestScene::OnRender()
	{
		...
		m_WaterTexture->Bind();
		{
			...
			glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(landPos.x, 0.04f * height + landPos.y, landPos.z));
